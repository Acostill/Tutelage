...
const updateSingleUser = (req, res, next) => {
  ...
  let query =
    "UPDATE users SET username = ${username}, firstname = ${firstname}, lastname = ${lastname}, password_digest = ${password}, ismentor = ${ismentor} WHERE id = ${id}";
  db
    .none(query, {
...
const createUser = (req, res, next) => {
  ...
  db
    .none(
      "INSERT INTO users (username, firstname, lastname, email, password_digest, ismentor) VALUES (${username}, ${firstname},${lastname}, ${password}, ${ismentor})",
      {
        username: req.body.username,
        firstname: req.body.firstname,
        lastname: req.body.lastname,
...
const getAllSurveyQuestionsAndAnswers = (req,res,next) => {
  db
    .any("SELECT the_question, answer_1, answer_2, answer_3, answer_4 FROM answers JOIN questions ON answers.id = answers.question_id")
    .then(function(data) {
      res.status(200).json({
        status: "success",
        data: data,
